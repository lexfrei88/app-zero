...
import pl.lexfrei.config.KafkaProperties;

class AppzeroKafkaResourceIT {
    ...
    void setup() {
        ...
        AppzeroKafkaResource kafkaResource = new AppzeroKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/appzero-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/appzero-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
